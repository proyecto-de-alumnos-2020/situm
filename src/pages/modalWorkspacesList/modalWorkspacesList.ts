...
import { Platform, NavParams, ViewController, AlertController, VirtualScroll, Alert, ModalController } from 'ionic-angular';
...
export class ModalWorkspacesList {
  ...
  deleteFromFirebase() {
    this.workspaceService.deleteWorkspaces(this.workspaces[0]).then(result => {
      if (result) {
  ...
  justOwnerCanOpen(){
    this.alertText("No editable", "El estado actual del workspace no permite edición.");
  }
  ...
  showInformation(){
    let modalInformation = this.modalCtrl.create(ModalInformation, { cssClass: 'modal-not-fullscreen-size' });
